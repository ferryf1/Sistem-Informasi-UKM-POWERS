...
<link rel="stylesheet" href="{{asset('front/css/comments.css')}}">
<div class="kom">
...
<div id="comment-{{ $comment->getKey() }}" class="media">
    ...
    <li id="comment-{{ $comment->getKey() }}" class="media">
        ...
        <img class="mr-3 lingkar"
            src="{{asset('gambar/white-square.png')}}"
            alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">
        <div class="media-body">
            <h5 class="mt-0 mb-1 author">{{ $comment->commenter->name ?? $comment->guest_name }} <small
                    class="text-muted">- {{ $comment->created_at->diffForHumans() }}</small></h5>
            <div style="white-space: pre-wrap;" class="komen">{!! $markdown->line($comment->comment) !!}</div>
